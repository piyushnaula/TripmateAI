...
def parse_route(query: str):
    ...
    # Pattern: "X trip from Y" / "X vacation from Y" (e.g. "Thailand trip from Bangladesh")
    match = re.search(
        r"(.+?)\s+(?:trip|tour|vacation|holiday|travel|visit)\s+from\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)",
        q_lower,
    )
    ...
    # Pattern: flights from X — but also check for destination mentions elsewhere
    ...
    if match:
        ...
        # Look for other location mentions in the query as a potential destination
        mentions = find_location_mentions(q)
        origin_resolved = origin_text.strip().lower()
        other_mentions = [m for m in mentions if m != origin_resolved]

        if other_mentions:
            arr_iata = resolve_location_to_iata(other_mentions[0])
            if dep_iata and arr_iata:
                return dep_iata, arr_iata

        return dep_iata, None

    # Pattern: flights to X — but also check for origin mentions elsewhere
    ...
    if match:
        ...
        # Look for other location mentions in the query as a potential origin
        mentions = find_location_mentions(q)
        dest_resolved = dest_text.strip().lower()
        other_mentions = [m for m in mentions if m != dest_resolved]

        if other_mentions:
            dep_iata = resolve_location_to_iata(other_mentions[0])
            if dep_iata and arr_iata:
                return dep_iata, arr_iata
